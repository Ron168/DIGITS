...
import digits.dataset.images.multi.test_views
import digits.dataset.multi.test_views
...
class BaseViewsTestWithAnyDataset(BaseViewsTest):
    """
    Provides a dataset
    This is a common interface to work with either "images/multi"
    datasets or "multi" datasets. The dataset type to use is chosen
...
class BaseViewsTestWithDataset(BaseViewsTestWithAnyDataset,
                               digits.dataset.images.multi.test_views.BaseViewsTestWithDataset):
    """
    This inherits from BaseViewsTestWithAnyDataset and
    digits.dataset.images.multi.test_views.BaseViewsTestWithDataset
    in order to provide an interface to test models on "images/multi" datasets
    """
    pass
...
class BaseTestViews(BaseViewsTest):
    ...
    def test_page_model_new(self):
        rv = self.app.get('/models/images/multi/new')
...
class BaseTestCreation(BaseViewsTestWithDataset):
    ...
    def infer_one_for_job(self, job_id):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_one?job_id=%s' % job_id,
            data={
...
class BaseTestCreatedWithAnyDataset(BaseViewsTestWithModelWithAnyDataset):
    ...
    def test_infer_one(self):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_one?job_id=%s' % self.model_id,
            data={
                'image_file': image_upload,
                'show_visualizations': 'y',
            }
    ...
    def test_infer_one_json(self):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_one.json?job_id=%s' % self.model_id,
            data={
                'image_file': image_upload,
            }
        )
    ...
    def test_infer_many(self):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_many?job_id=%s' % self.model_id,
    ...
    def test_infer_db(self):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_db?job_id=%s' % self.model_id,
    ...
    def test_infer_many_json(self):
        ...
        rv = self.app.post(
            '/models/images/multi/infer_many.json?job_id=%s' % self.model_id,
            data={'image_list': file_upload}
        )
        assert rv.status_code == 200, 'POST failed with %s' % rv.status_code
        data = json.loads(rv.data)
    ...
    def test_infer_db_json(self):
        if self.val_db_path is None:
            raise unittest.SkipTest('Class has no validation db')
        rv = self.app.post(
            '/models/images/multi/infer_db.json?job_id=%s' % self.model_id,
            data={'db_path': self.val_db_path}
        )
...
class BaseTestCreated(BaseTestCreatedWithAnyDataset,
                      digits.dataset.images.multi.test_views.BaseViewsTestWithDataset):
    """
    Tests on a model that has already been created with an "images/multi" dataset
    """
    pass


class BaseTestCreatedWithGradientDataExtension(BaseTestCreatedWithAnyDataset,
                                               digits.dataset.multi.test_views.BaseViewsTestWithDataset):
    """
    Tests on a model that has already been created with a "multi" dataset,
    ...
    def test_infer_extension_json(self):
        rv = self.app.post(
            '/models/images/multi/infer_extension.json?job_id=%s' % self.model_id,
            data={
                'gradient_x': 0.5,
                'gradient_y': -0.5,
            }
