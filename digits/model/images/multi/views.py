...
from .forms import MultiImageModelForm
from .job import MultiImageModelJob
from digits.pretrained_model.job import PretrainedModelJob
from digits import extensions, frameworks, utils
from digits.config import config_value
from digits.dataset import MultiDatasetJob, MultiImageDatasetJob
...
def new(extension_id=None):
    """
    Return a form for a new MultiImageModelJob
    """
    form = MultiImageModelForm()
...
def create(extension_id=None):
    """
    Create a new MultiImageModelJob
...
def large_graph():
    ...
    return flask.render_template('models/images/multi/large_graph.html', job=job)
...
def infer_one():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/images/multi/infer_one.html',
            model_job=model_job,
            job=inference_job,
            image_src=image,
...
def infer_extension():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/images/multi/infer_extension.html',
            model_job=model_job,
            job=inference_job,
            keys=keys,
...
def infer_db():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/images/multi/infer_db.html',
            model_job=model_job,
            job=inference_job,
            keys=keys,
            inference_views_html=inference_views_html,
...
def infer_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/images/multi/infer_many.html',
            model_job=model_job,
            job=inference_job,
            paths=paths,
            inference_views_html=inference_views_html,
...
def get_datasets(extension_id):
    if extension_id:
        jobs = [j for j in scheduler.jobs.values()
                if isinstance(j, MultiDatasetJob)
                and j.extension_id == extension_id
                and (j.status.is_running() or j.status == Status.DONE)]
    else:
        jobs = [j for j in scheduler.jobs.values()
                if (isinstance(j, MultiImageDatasetJob) or isinstance(j, MultiDatasetJob))
                and (j.status.is_running() or j.status == Status.DONE)]
    return [(j.id(), j.name())
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, MultiImageModelJob)],
