...
@subclass
class MultiDatasetJob(DatasetJob):
    """
    ...
    def __setstate__(self, state):
        super(MultiDatasetJob, self).__setstate__(state)
        self.pickver_job_dataset_extension = PICKLE_VERSION
    ...
    def json_dict(self, verbose=False):
        d = super(MultiDatasetJob, self).json_dict(verbose)
        if verbose:
            d.update({
                'create_db_tasks': [{
                    "name": t.name(),
