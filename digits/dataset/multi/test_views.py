...
class BaseViewsTestWithDataset(BaseViewsTest):
    ...
    def create_dataset(cls, **kwargs):
        ...
        **kwargs -- data to be sent with POST request
        ...
        url = '/datasets/multi/create/%s' % cls.EXTENSION_ID
        if request_json:
            url += '.json'
...
class MultiViewsTest(BaseViewsTest):

    @classmethod
    def setUpClass(cls, **kwargs):
        if extensions.data.get_extension(cls.EXTENSION_ID) is None:
            raise unittest.SkipTest('Extension "%s" is not installed' % cls.EXTENSION_ID)
        super(MultiViewsTest, cls).setUpClass()

    def test_page_dataset_new(self):
        rv = self.app.get('/datasets/multi/new/%s' % self.EXTENSION_ID)
        print rv.data
        assert rv.status_code == 200, 'page load failed with %s' % rv.status_code
...
class MultiCreationTest(BaseViewsTestWithDataset):
    """
...
class MultiCreatedTest(BaseViewsTestWithDataset):
    """
    ...
    def test_explore_features(self):
        ...
        rv = self.app.get('/datasets/multi/explore?db=train_db%%2Ffeatures&job_id=%s' % self.dataset_id)
...
class TestImageGradientViews(MultiViewsTest, test_utils.DatasetMixin):
    """
...
class TestImageGradientCreation(MultiCreationTest, test_utils.DatasetMixin):
    """
...
class TestImageGradientCreated(MultiCreatedTest, test_utils.DatasetMixin):
    """
    Test that create datasets
    """
    EXTENSION_ID = "image-gradients"
...
class TestImageProcessingCreated(MultiCreatedTest, test_utils.DatasetMixin):
    """
...
class TestImageProcessingCreatedWithSeparateValidationDirs(MultiCreatedTest, test_utils.DatasetMixin):
    """
