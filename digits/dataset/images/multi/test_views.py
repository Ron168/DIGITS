zz# Copyright (c) 2015-2017, NVIDIA CORPORATION.  All rights reserved.
    ...
    def create_dataset(cls, **kwargs):
        ...
        **kwargs -- data to be sent with POST request
        ...
        data = {
            ...
            'method':           'prebuilt',
            'prebuilt_train_images_0': os.path.join(cls.imageset_folder, 'train_images_0'),
            'prebuilt_train_images_1': os.path.join(cls.imageset_folder, 'train_images_1'),
            'prebuilt_train_labels': os.path.join(cls.imageset_folder, 'train_labels'),
            'prebuilt_val_images_0': os.path.join(cls.imageset_folder, 'val_images_0'),
            'prebuilt_val_images_1': os.path.join(cls.imageset_folder, 'val_images_1'),
            'prebuilt_val_labels': os.path.join(cls.imageset_folder, 'val_labels'),
            'prebuilt_mean_file_0': os.path.join(cls.imageset_folder, 'train_mean_0.binaryproto'),
            'prebuilt_mean_file_1': os.path.join(cls.imageset_folder, 'train_mean_1.binaryproto'),
        }
        data.update(kwargs)

        request_json = data.pop('json', False)
        url = '/datasets/images/multi'
        if request_json:
            url += '.json'
...
class TestViews(BaseViewsTest, test_utils.DatasetMixin):
    ...
    def test_page_dataset_new(self):
        rv = self.app.get('/datasets/images/multi/new')
