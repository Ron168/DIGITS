...
@subclass
class MultiImageDatasetJob(ImageDatasetJob):
    """
    A Job that creates an image dataset for a multi network
    """

    def __init__(self, **kwargs):
        self.mean_file_0 = kwargs.pop('mean_file_0', None)
        self.mean_file_1 = kwargs.pop('mean_file_1', None)
        super(MultiImageDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_image_multi = PICKLE_VERSION

    def __setstate__(self, state):
        super(MultiImageDatasetJob, self).__setstate__(state)
        self.pickver_job_dataset_image_multi = PICKLE_VERSION
    ...
    def get_feature_db_paths(self, stage):
        ...
        for task in self.tasks:
            if ('%s Images' % s) in task.purpose:
                dbs.append(self.path(task.database))
